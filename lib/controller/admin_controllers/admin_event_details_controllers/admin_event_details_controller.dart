...
class AdminEventDetailsController {
  ...
  // Dependencies for Demographic Questions
  final QuestionSetsController _questionSetsController =
      Get.put(QuestionSetsController());
  ...
  // Observable list for question sets
  RxList<QuestionSet> availableQuestionSets = <QuestionSet>[].obs;
  ...
  void _loadQuestionSets() {
    // Listen to the stream of question sets created by the organizer
    _questionSetsController.streamQuestionSets().listen((sets) {
      availableQuestionSets.value = sets;
    });
  }
  ...
  void removeMenuItemFromSelection(MenuItem item) {
    selectedMenusLocally.value = selectedMenusLocally.value
        .where((i) => i.menuItemId != item.menuItemId)
        .toList();
    event?.selectedMenus?.remove(item.menuItemId);
  }
  ...
  Future<void> updateEvent() async {
    if (event == null) throw Exception('Cannot update event: no event loaded');
    if (event!.eventId == null)
      throw Exception('Cannot update event: eventId is null');
